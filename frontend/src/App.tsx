...
function viewFromLocation(): View {
  ...
  if (path === '/phase-two-osrs-preview' || path.startsWith('/phase-two-osrs-preview/')) return { name: 'phase2OsrsPreview' };
  return { name: 'home' };
}
...
function feedbackContext(view: View): string {
  switch (view.name) {
    ...
    case 'setup':
      return 'page:setup';
    case 'phase2OsrsPreview':
      return 'page:phase-two-osrs-preview';
    default:
      return 'page:home';
  }
}
      ...
      next.name === 'player'
        ...
        : next.name === 'boss'
          ...
          : next.name === 'faq'
            ...
            : next.name === 'setup'
              ? '/setup'
              : next.name === 'phase2OsrsPreview'
                ? '/phase-two-osrs-preview'
              : '/';
    window.history.pushState({}, '', path);
    setView(next);
  };

  if (view.name === 'phase2OsrsPreview') {
